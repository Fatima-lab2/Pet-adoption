...
@endif
    <div class="container">
        <h2>Contracts for {{ ucfirst($employee->name) }}</h2>
            @role('employee')
            @if(Auth::user()?->employee?->department_id == 2)
        <div style="text-align: right; margin-bottom: 15px;">
            <a href="{{ url('/add_employee_contract/' . $employee->employee_id) }}" class="btn btn-add" style="display: inline-block; background-color: #3d3dd0; color: white; padding: 10px 20px; font-size: 16px; text-decoration: none; border-radius: 5px; transition: background-color 0.3s;"  class="btn btn-add">Add New Contract</a>
        </div>
            @endif
            @endrole
        @forelse($contracts as $contract)
            <div class="contract-box">
                ...
                @else
                    <div class="actions">
                        <a href="{{ url('/employee_terminate_contract/' . $contract->contract_id) }}" class="btn btn-terminate">Terminate Contract</a>
                    </div>
                @endif
            </div>
        @empty
